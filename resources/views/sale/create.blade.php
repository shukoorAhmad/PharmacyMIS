        ...
        <div class="card">
            <div class="card-body">
                <div class="d-flex justify-content-between">
                    <h4 class="card-title">{{ __('words.New Sale') }}</h4>
                    <a href="{{ route('sale-list') }}" class="btn btn-primary mb-4">{{ __('words.Sales List') }}</a>
                </div>
                <form method="POST" action="{{ route('sale-store') }}">
                    @csrf
                    <div class="row">
                        <div class="col-md-4">
                            <label>{{ __('words.Select Sale Type') }}</label>
                            <select name="sale_type" id="sale_type" class="form-control select2" style="width: 100% !important;" required>
                                <option value="" selected disabled>{{ __('words.Select Sale Type') }}</option>
                                <option value="1">{{ __('words.Customer') }}</option>
                                <option value="2">{{ __('words.Seller') }}</option>
                            </select>
                        </div>
                        <div class="col-md-4">
                            <label id="show-label-name" class="w-100">{{ __('words.Select') }}</label>
                        ...
                        <div class="col-md-4 visibility-hidden" id="percentage">
                            <label>{{ __('words.Seller Percentage') }}</label>
                            <select name="percentage" class="form-control select2" style="width: 100% !important;">
                                <option value="" selected disabled>{{ __('words.Select Percentage') }}</option>
                                @for ($i = 1; $i <= 10; $i++)
                    ...
                    <div class="col-md-12 form-group p-0">
                        <hr>
                        <label class="w-100">{{ __('words.Select Item') }}</label>
                        <select name="item_id" id="item_id" class="form-control" style="width: 100% !important;">
                            <option value="" selected disabled>{{ __('words.Select Item') }}</option>
                        </select>
                    </div>
                    ...
                    <div class="p-0 m-0 d-none" id="sale-div">
                        <div class="row">
                            <div class="form-group col-md-2">
                                <label>{{ __('words.Sale Date') }}</label>
                                <input type="date" class="form-control" name="sale_date" value="<?php echo date('Y-m-d'); ?>">
                            </div>
                            <div class="form-group col-md-3">
                                <label>{{ __('words.Total Bill (AFG)') }}</label>
                                <input type="text" class="form-control" id="total_bill" value="0" name="total">
                            </div>
                            <div class="form-group col-md-3">
                                <label>{{ __('words.Pay Amount (AFG)') }}</label>
                                <input type="text" class="form-control" name="paid_amount" value="0">
                            </div>
                            <div class="form-group col-md-4">
                                <label>{{ __('words.Comment') }}</label>
                                <input type="text" class="form-control" name="comment" placeholder="{{ __('words.Write comments Here if you have...') }}">
                            </div>

                        </div>
                        <button type="submit" class="btn btn-primary mt-4">{{ __('words.Sale') }}</button>
                    </div>
                    <!-- </div> -->
                </form>
            </div>
