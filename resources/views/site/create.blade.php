...
@section('content')
    ...
    <div class="col-12 box-margin height-card">
        <div class="card">
            <div class="card-body">
                <h4 class="card-title" style="cursor: pointer;">New Site</h4>
                <form method="POST" action="{{ route('sitestore') }}">
                    @csrf
                    <div class="form-row">
                        <div class="form-group col-md-6">
                            <label class="col-form-label">Site</label>
                            <input class="form-control  @error('site_name') is-invalid @enderror" name="site_name"
                                value="{{ old('site_name') }}" placeholder="Write Your Site Here..." autofocus required>
                            @error('site_name')
                        ...
                        <div class="form-group col-md-6">
                            ...
                            <select name="province" class="form-control select2 @error('province') is-invalid @enderror"
                                required>
                                <option value="" selected disabled>Please Select Province</option>
                                @foreach ($province as $pro)
                                    <option value="{{ $pro->province_id }}"
                                        {{ old('province') == $pro->province_id ? 'selected' : '' }}>
                                        {{ $pro->en_province }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <tbody>
                        @foreach ($site as $key => $s)
                            <tr>
                                ...
                                <th><a data-id="{{ $s->site_id }}" class="edit"><i
