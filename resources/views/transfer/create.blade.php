        ...
        <div class="card">
            <div class="card-body">
                <div class="d-flex justify-content-between">
                    <h4 class="card-title">{{ __('words.New Transfer') }}</h4>
                    <a href="{{ route('transfer-list') }}" class="btn btn-success mb-3">{{ __('words.Transfers List') }}</a>
                </div>
                <form method="POST" action="{{ route('transfer-store') }}">
                    @csrf
                    <div class="row">
                        <div class="form-group col-md-3">
                            <label class="col-form-label">{{ __('words.From Stock') }}</label>
                            <select name="source_stock_id" id="source_stock_id" class="form-control select2" required>
                                <option value="" selected disabled>{{ __('words.Select From stock') }}</option>
                        ...
                        <div class="form-group col-md-3">
                            <label class="col-form-label">{{ __('words.To Stock') }}</label>
                            <select name="destination_stock_id" id="destination_stock_id" class="form-control select2" required>
                                <option value="" selected disabled>{{ __('words.Select Destination stock') }}</option>
                            </select>
                        ...
                        <div class="form-group col-md-2">
                            <label class="col-form-label">{{ __('words.Transfer Date') }}</label>
                        ...
                        <div class="form-group col-md-4">
                            <label class="col-form-label">{{ __('words.Comment') }}</label>
                            <input class="form-control  @error('comment') is-invalid @enderror" name="comment" value="{{ old('comment') }}" placeholder="{{ __('words.Write comments Here if you have...') }}">
                            @error('comment')
                                <span class="invalid-feedback" role="alert">
                                    <strong> {{ $message }} </strong>
                                </span>
